 ...
 * Uses office-bg.png as the base scene
 * Researchers are positioned at desks with status indicators
...
// Desk positions in the office (percentage-based for responsiveness)
...
const STATUS_COLORS: Record<string, string> = {
  idle: 'oklch(0.55 0.02 260)',
  researching: 'oklch(0.65 0.2 265)',
  completed: 'oklch(0.72 0.19 155)',
  failed: 'oklch(0.63 0.22 25)',
};
...
function ResearcherSprite({ researcher, position, onSelect }: {
  ...
  // Typing animation for researching state
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Status bubble (always visible when researching, hover for others) */}
      <AnimatePresence>
        {(isHovered || researcher.status === 'completed' || researcher.status === 'researching') && (
          ...
          >
            <div
              ...
              style={{ borderColor: STATUS_COLORS[researcher.status] }}
            >
              <p className="font-pixel text-[7px] text-[oklch(0.92_0.01_260)] mb-0.5 flex items-center gap-1">
                {researcher.name}
                <span className="text-[6px]" style={{ color: STATUS_COLORS[researcher.status] }}>
                  Lv.{researcher.level}
                </span>
              </p>
              <p className="font-pixel text-[6px]" style={{ color: STATUS_COLORS[researcher.status] }}>
                {researcher.status === 'researching'
                  ? `${researcher.currentTask}${typingDots}`
                  : STATUS_LABELS[researcher.status]
                }
              </p>
              {researcher.status === 'researching' && (
                <div className="mt-1 w-full h-[5px] bg-[oklch(0.18_0.02_260)] border border-[oklch(0.3_0.03_260)]">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: STATUS_COLORS[researcher.status],
                    }}
                  />
                </div>
              )}
              ...
              <div
                ...
                style={{ backgroundColor: 'oklch(0.1 0.015 260)', borderRight: `2px solid ${STATUS_COLORS[researcher.status]}`, borderBottom: `2px solid ${STATUS_COLORS[researcher.status]}` }}
              />
            </div>
          </motion.div>
        )}
      ...
      <motion.div
        ...
        style={{
          filter: isHovered
            ? `drop-shadow(0 0 10px ${STATUS_COLORS[researcher.status]})`
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundColor: researcher.mood > 70
              ? 'oklch(0.72 0.19 155)'
              : researcher.mood > 40
              ? 'oklch(0.82 0.15 85)'
              : 'oklch(0.63 0.22 25)',
      ...
      <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 whitespace-nowrap">
        <span
          ...
          style={{
            color: isHovered ? STATUS_COLORS[researcher.status] : 'oklch(0.65 0.02 260)',
            borderColor: isHovered ? `${STATUS_COLORS[researcher.status]}60` : 'transparent',
          }}
        >
          {researcher.name.length > 6 ? researcher.name.slice(0, 6) : researcher.name}
        </span>
      </div>
    </div>
  );
      ...
      <div className="relative w-full h-full max-w-[1200px] max-h-[700px] mx-auto">
        ...
        <div className="absolute top-3 right-4 z-10">
          <div className="bg-[oklch(0.08_0.015_260_/_0.88)] border-2 border-[oklch(0.28_0.03_260)] px-3 py-1.5 backdrop-blur-sm">
            ...
            <p className="font-pixel text-[5px] text-[oklch(0.4_0.02_260)] mt-0.5">
              研究员: {state.researchers.length}/6 · Lv.{state.companyLevel}
            </p>
          </div>
        </div>
        ...
        <div className="absolute bottom-3 left-4 flex gap-2 z-10">
          {[
            ...
            { label: '实盘', value: state.strategies.filter(s => s.status === 'live').length, color: 'oklch(0.82 0.15 85)', bg: 'oklch(0.82 0.15 85 / 0.08)' },
          ].map(stat => (
            <div
              key={stat.label}
